...
def global_explore(search_tree, dynamics_model, sample_state=None, steer_type='line', controller=None,
    ...
    while not finish_sample:
        ...
        nearest_tree_idx = search_tree.non_terminal_idxes[nearest_idx]
        if search_tree.vert_radius[nearest_tree_idx] > np.linalg.norm(sample_state - non_terminal_states[nearest_idx]):
            finish_sample = True
        else:
            sample_state = dynamics_model.sample_state_space(1).numpy().squeeze()
    t1 = time.time()
